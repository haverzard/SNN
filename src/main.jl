...
function create_layer(from::Int, to::Int)::Layer
    ...
    random_bias = rand(to)
    return Layer(random_weights, (sum(random_weights, dims=1)[1:end] .+ random_bias), random_bias, to, from)
end

function goto_layer(input::Array{Float64,1}, layer::Layer)::Array{Float64,1}
    result = (sum(input .* layer.neurons_weights, dims=1)[1:end] .+ layer.neurons_bias) ./ layer.neurons_weights_total
    result[result .< 0] .= 0
    return result
end